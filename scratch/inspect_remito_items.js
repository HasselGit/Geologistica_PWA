...
async function main() {
  try {
    const data = await getJson('https://suwcqdlxnmfcvmlnzizl.supabase.co/rest/v1/remito_items?select=*');
    console.log('--- REMITO ITEMS ---');
    console.log(JSON.stringify(data, null, 2));
  } catch (e) {
    console.error('Error (table probably does not exist or has RLS):', e.message);
  }
}
